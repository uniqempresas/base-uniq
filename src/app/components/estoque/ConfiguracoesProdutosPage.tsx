...
import {
  ...
  Shapes, Pencil, X, Check,
} from "lucide-react";
import { toast } from "sonner";
import { useCategorias, type CategoriaProduto } from "../../hooks/use-categorias";
import { CORES_TAG, getTagPalette } from "../../hooks/use-tags";

/**
 * Configurações de Produtos — CRUD de categorias.
 *
 * Espelha `ConfiguracoesCRMPage` (tags/etiquetas de cliente), que é o padrão
 * aprovado pelo fundador. Diferenças deliberadas, listadas no WIRE:
 *  - botão EDITAR (tags não têm — o pedido foi "cadastrar E editar")
 *  - remoção é SOFT DELETE (igual às tags): a categoria sai dos selects, mas o
 *    produto que já a usava mantém o rótulo
 *  - aviso com a contagem de produtos antes de remover
 */
export function ConfiguracoesProdutosPage() {
  const navigate = useNavigate();
  const {
    categorias, loading, error, isFallback, recarregar,
    criarCategoria, atualizarCategoria, desativarCategoria, contarProdutos,
  } = useCategorias();
  ...
  const [aRemover, setARemover] = useState<{ categoria: CategoriaProduto; total: number } | null>(null);
  ...
  const handleAdicionar = async () => {
    ...
    setCriando(true);
    const res = await criarCategoria(nomeLimpo, cor);
    setCriando(false);
    if (res.success) {
  ...
  const salvarEdicao = async () => {
    ...
    const res = await atualizarCategoria(editandoId, { nome: nomeLimpo, cor: editCor });
  ...
  const pedirRemocao = async (cat: CategoriaProduto) => {
    const total = await contarProdutos(cat.id);
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-[#efefef] shadow-sm p-5 sm:p-6">
        <div className="flex items-start gap-3 mb-5">
          <div className="w-10 h-10 rounded-xl bg-[#efefef] flex items-center justify-center shrink-0">
            <Shapes size={18} className="text-[#627271]" />
          </div>
          <div>
            ...
            <p className="text-[#627271] text-xs mt-0.5">
              As categorias agrupam os produtos no estoque e alimentam a barra de categorias da loja virtual.
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-[#627271] text-xs">
              Adicione a primeira categoria acima para começar a organizar seus produtos.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
      ...
      <p className="text-[#627271] text-xs px-1">
        Categorias removidas deixam de aparecer em novos cadastros, mas continuam valendo nos produtos já cadastrados.
      </p>
      ...
      {aRemover && (
        ...
        >
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="titulo-remover-categoria"
          >
            <h2 id="titulo-remover-categoria" className="text-[#1f2937] text-base mb-3" style={{ fontWeight: 700 }}>
              Remover “{aRemover.categoria.nome}”?
            </h2>
            <p className="text-[#627271] text-sm mb-5">
              {aRemover.total > 0
                ? `${aRemover.total} produto${aRemover.total !== 1 ? "s" : ""} usa${aRemover.total !== 1 ? "m" : ""} esta categoria. Ela deixa de aparecer em novos cadastros, mas os produtos mantêm a categoria.`
                : "Nenhum produto usa esta categoria. Ela deixará de aparecer em novos cadastros."}
            </p>
