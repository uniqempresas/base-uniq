...
| B13 | **Dashboard é 100% mock — e faz afirmações falsas sobre o negócio** | 🔎 **Achado em 23/09/2026.** O login redireciona para **`/dashboard`** (`LoginPage.tsx:45`), então esta é a **primeira tela** que o parceiro vê. E ela **não lê nada do banco**: o único hook é `useAuth` (`DashboardPage.tsx:137`) — **zero chamadas ao Supabase**. Tudo é fixo: `metaValor = 2000` e `metaAtual = 1250` (`:167-168`), `trendValue="+20%"` / `"+50%"` / `"+1 vs ontem"` (`:206/216/226`), `12% esta semana` (`:243`), `40%` (`:479`). **O mais grave:** o array `MEL_INSIGHTS` (`:64`) faz **afirmações específicas e falsas** em nome da MEL — *"Você vendeu 20% mais que ontem"*, *"ligar para o João Silva — follow-up às 14h"*, *"Camiseta P está acabando (2 unidades)"* e *"a R$ 750 da sua meta semanal"*. São dados do **protótipo original** (loja de roupas) — a Doceê vende **doces**. É a **mesma classe** do `ASSINATURA_MOCK`, do `PLANOS_COMPARATIVO` e do "UNIQ Pro Enterprise" já removidos, mas maior: é a porta de entrada e **atribui à MEL** coisas que não aconteceram. ⚠️ **Corrigir é decisão de produto** (quais KPIs o Dashboard mostra? qual a meta? qual período?), não só de código. | **Alta** |
