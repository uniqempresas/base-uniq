...
| 8 | 11/09/2026 | Dashboard (mobile) | Big numbers (cards KPI "Faturamento hoje R$ 1.250" etc.) ocupam espaço demais na tela do celular — precisa ajuste responsivo. Dados mockados podem continuar por hora | Média | ✅ Implementado (11/09) — KPIs compactos no mobile (padding/ícone/valor reduzidos, trend e subtítulo ocultos em telas pequenas); desktop inalterado. **Aguarda validação do fundador.** | — |
| 9 | 11/09/2026 | Dashboard (header) | Botão "+" na linha do nome de usuário = "Nova Venda" (placeholder). No mobile mostra só o ícone (`hidden sm:inline`) e hoje **não faz nada** — `setShowQuickSale(true)` seta um estado que nenhuma parte do componente lê. Botão "Venda rápida" do Acesso Rápido também é inerte | Média | ✅ Implementado (11/09) — "+"/"Nova Venda" e "Venda rápida" agora navegam para `/vendas/pedidos?novo=1`, que abre o modal "Novo Pedido" automaticamente (e limpa o param da URL). **Aguarda validação do fundador.** | — |
